...
def maze_to_dka(maze : Maze) -> DFA:
    ...
    transitions = {}
    start_state = (1, 1)
    accept_states = set([(0, i) for i in range(cols)] + [(rows-1, i) for i in range(cols)] + [(i, 0) for i in range(rows)] + [(i, cols-1) for i in range(cols)])
    states = set([(i, j) for i in range(rows) for j in range(cols)])
    alphabet = ['S', 'N', 'W', 'E']
...
def counter_word_transition(maze_dka : DFA, transition : tuple) -> str:
    return bfs(maze_dka, transition[0], False)[::-1] + transition[1] + bfs(maze_dka, maze_dka.transitions[transition], True)
